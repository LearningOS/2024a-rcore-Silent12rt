...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    if process_inner.dead_lock_enable {
        if process_inner.detect_deadlock( false) {
            let task = current_task().unwrap();
            let mut task_inner = task.inner_exclusive_access();
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    if process_inner.dead_lock_enable {
        if process_inner.detect_deadlock( true) {
            let task = current_task().unwrap();
            let mut task_inner = task.inner_exclusive_access();
...
pub fn sys_enable_deadlock_detect(_enabled: usize) -> isize {
    ...
    match _enabled {
        1 => current_process().inner_exclusive_access().dead_lock_enable = true,
        0 => current_process().inner_exclusive_access().dead_lock_enable = false,
        _ => return -1,
    };
    0
}